...
function AppShell({ children, mode }: { children: ReactNode; mode: 'control' | 'community' }) {
  ...
  const controlNav = [{ href: '/control', icon: Gauge, label: 'Overview' }, { href: '/control/reality', icon: Compass, label: 'Current reality' }, { href: '/control/projects', icon: FolderKanban, label: 'Projects' }, { href: '/control/issues', icon: CircleAlert, label: 'Open issues' }, { href: '/control/coverage', icon: Globe2, label: 'Coverage' }, { href: '/control/operations', icon: Layers3, label: 'Operations room' }];
  const communityNav = [{ href: '/community', icon: Sparkles, label: 'At a glance' }, { href: '/community/projects', icon: FolderKanban, label: 'Projects' }, { href: '/community/listen', icon: MessageSquare, label: 'Have your say' }, { href: '/community/events', icon: CalendarDays, label: 'Events' }, { href: '/community/operations', icon: Layers3, label: 'Community workspace' }];
  ...
  return <div className="grain min-h-[100dvh] bg-[#f1efe8] text-[#102c2b]">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-[#102c2b] px-5 py-6 text-[#f5f1e9] transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="mt-12 space-y-1" aria-label="Primary navigation">{nav.map(({ href, icon: Icon, label }) => <a key={`${href}-${label}`} href={href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 border-l-2 px-3 py-3 text-sm transition-colors ${currentHref === href ? 'border-[#6bd49d] bg-white/10 text-white' : 'border-transparent text-[#9bb3ac] hover:bg-white/5 hover:text-white'}`} data-testid={`link-${mode}-${label.toLowerCase().replaceAll(' ', '-')}`}><Icon className="h-4 w-4" />{label}</a>)}</nav>
...
function PortalPage({ mode, eyebrow, title, copy, children }: { mode: 'control' | 'community'; eyebrow: string; title: string; copy: string; children: ReactNode }) {
  return <AppShell mode={mode}><PageTransition><main className="mx-auto max-w-[1440px] px-5 py-9 sm:px-8 lg:px-12 lg:py-12"><div className="mb-9 max-w-3xl"><Eyebrow>{eyebrow}</Eyebrow><h1 className="mt-4 font-serif text-5xl tracking-[-0.04em] sm:text-6xl">{title}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-[#59756d]">{copy}</p></div>{children}</main></PageTransition></AppShell>;
}

function ControlRealityPage() {
  return <PortalPage mode="control" eyebrow="Mission control / current reality" title="What needs the room." copy="A focused view of the signals shaping today’s decisions across Homabay County."><RealityPanel /></PortalPage>;
}

function ControlIssuesPage() {
  return <PortalPage mode="control" eyebrow="Mission control / open issues" title="Signals that need a response." copy="Review the highest-attention issues, their evidence, and the latest community signal counts."><RealityPanel /></PortalPage>;
}

function ControlProjectsPage() {
  const [project, setProject] = useState<Project | null>(null);
  return <PortalPage mode="control" eyebrow="Mission control / project pipeline" title="The work, in motion." copy="Track owners, progress, targets, and the next practical step for every Homabay County project."><ProjectPipeline onSelect={setProject} /><ProjectDrawer project={project} onClose={() => setProject(null)} /></PortalPage>;
}

function ControlCoveragePage() {
  return <PortalPage mode="control" eyebrow="Mission control / regional coverage" title="Where the signal is coming from." copy="Compare aggregated participation across the county while protecting individual privacy."><CoveragePanel /></PortalPage>;
}

function CommunityAtAGlancePage() {
  const project = repo.getProjects()[0];
  return <AppShell mode="community"><PageTransition><main><section className="bg-[#102c2b] px-5 py-16 text-[#f5f1e9] sm:px-8 lg:px-12 lg:py-24"><div className="mx-auto max-w-[1440px]"><Eyebrow light>Homabay 2026 / Homabay County</Eyebrow><h1 className="mt-7 max-w-4xl font-serif text-[clamp(3.5rem,8vw,8rem)] leading-[.85] tracking-[-0.06em]">A county that<br /><em className="text-[#6bd49d]">works in the open.</em></h1><p className="mt-8 max-w-lg text-lg leading-7 text-[#b6c9c0]">See what is moving in Homabay County, what we are learning, and where your voice can help shape the next step.</p></div></section><section className="mx-auto grid max-w-[1440px] gap-4 px-5 py-12 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:px-12 lg:py-20"><Panel className="bg-[#dcecdf] p-6 sm:p-9"><Eyebrow>Featured project</Eyebrow><h2 className="mt-4 font-serif text-4xl">{project.name}</h2><p className="mt-5 text-base leading-7 text-[#31574d]">{project.summary}</p><div className="mt-8"><div className="flex justify-between text-xs"><span>Current progress</span><span>{project.progress}%</span></div><div className="mt-3"><ProgressBar value={project.progress} /></div></div></Panel><Panel className="p-6 sm:p-9" dark><Eyebrow light>Latest update</Eyebrow><h2 className="mt-4 font-serif text-3xl">The next stop is Mbita.</h2><p className="mt-4 text-sm leading-6 text-[#b6c9c0]">The route team is meeting riders in Mbita to understand the final connection home for late shifts.</p></Panel></section></main></PageTransition></AppShell>;
}

function CommunityProjectsPage() {
  const [project, setProject] = useState<Project | null>(null);
  return <PortalPage mode="community" eyebrow="Community portal / the work" title="See the promise take shape." copy="Every project has a current state, a next step, and a person accountable for moving it forward."><ProjectPipeline onSelect={setProject} /><ProjectDrawer project={project} onClose={() => setProject(null)} /></PortalPage>;
}

function CommunityListenPage() {
  const [voted, setVoted] = useState<Record<string, string>>({});
  const vote = (pollId: string, option: string) => setVoted((current) => ({ ...current, [pollId]: option }));
  return <PortalPage mode="community" eyebrow="Community portal / have your say" title="Small signals, real direction." copy="Answer a question or share a practical signal from your day. Responses are grouped with others before the campaign team sees them."><div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]"><Panel className="p-6 sm:p-9"><div className="flex items-center justify-between"><Eyebrow>Open questions</Eyebrow><MessageSquare className="h-5 w-5 text-[#4caa79]" /></div><div className="mt-8 space-y-4">{repo.getPolls().map((poll) => <PollCard key={poll.id} poll={poll} voted={Boolean(voted[poll.id])} onVote={vote} />)}</div></Panel><Panel className="p-6 sm:p-9" dark><Eyebrow light>Why we ask</Eyebrow><h2 className="mt-4 font-serif text-3xl">Participation without the machinery.</h2><div className="mt-8 space-y-6">{repo.getManifesto().map((item) => <div key={item.id} className="border-t border-white/15 pt-5"><span className="font-mono text-[10px] text-[#6bd49d]">{item.number}</span><h3 className="mt-2 font-semibold">{item.title}</h3><p className="mt-1 text-sm leading-5 text-[#9bb3ac]">{item.body}</p></div>)}</div></Panel></div></PortalPage>;
}

function CommunityEventsPage() {
  const [registered, setRegistered] = useState<Record<string, boolean>>({});
  return <PortalPage mode="community" eyebrow="Community portal / in the neighborhood" title="Make it a room." copy="No stage, no pitch. Just the next useful conversation for neighbors across Homabay County."><Panel className="p-6 sm:p-9"><div className="flex items-center justify-between"><div><Eyebrow>Upcoming gatherings</Eyebrow><h2 className="mt-4 font-serif text-4xl">Meet the work where it lives.</h2></div><CalendarDays className="h-5 w-5 text-[#4caa79]" /></div><div className="mt-8">{repo.getEvents().map((event) => <EventCard key={event.id} event={event} registered={Boolean(registered[event.id])} onRegister={() => setRegistered((current) => ({ ...current, [event.id]: true }))} />)}</div></Panel></PortalPage>;
}

function OperationsPage({ mode }: { mode: 'control' | 'community' }) {
  return <AppShell mode={mode}><PageTransition><main className="mx-auto max-w-[1440px] px-5 py-9 sm:px-8 lg:px-12 lg:py-12"><div className="max-w-3xl"><Eyebrow>{mode === 'control' ? 'Mission control / operations room' : 'Community portal / shared workspace'}</Eyebrow><h1 className="mt-4 font-serif text-5xl tracking-[-0.04em] sm:text-6xl">{mode === 'control' ? 'Run the whole campaign.' : 'Shape the work together.'}</h1><p className="mt-4 text-sm leading-6 text-[#59756d]">{mode === 'control' ? 'Manage teams, relationships, projects, community groups, tasks, and public communications from one synchronized workspace.' : 'Follow the groups, projects, and communications that shape everyday life in Homabay County.'}</p></div><OperationsWorkspace mode={mode} /></main></PageTransition></AppShell>;
}

function Router() {
  return <Switch><Route path="/" component={Landing} /><Route path="/control/reality" component={ControlRealityPage} /><Route path="/control/projects" component={ControlProjectsPage} /><Route path="/control/issues" component={ControlIssuesPage} /><Route path="/control/coverage" component={ControlCoveragePage} /><Route path="/control/operations" component={() => <OperationsPage mode="control" />} /><Route path="/control" component={ControlPage} /><Route path="/community/projects" component={CommunityProjectsPage} /><Route path="/community/listen" component={CommunityListenPage} /><Route path="/community/events" component={CommunityEventsPage} /><Route path="/community/operations" component={() => <OperationsPage mode="community" />} /><Route path="/community" component={CommunityAtAGlancePage} /><Route component={NotFound} /></Switch>;
}
